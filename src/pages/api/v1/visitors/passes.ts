...
// POST /api/v1/visitors/passes — create a new time-bound visitor pass
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const sb = getSupabaseServiceClient();
    const rules = await getRules(sb, SOCIETY_ID, ['VISITOR_PASS_MAX_HOURS', 'VISITOR_PASS_MAX_USES_DEFAULT']);
    ...
    const maxUses = Math.max(1, Math.min(body.max_uses ?? 1, 50));
    ...
    const { data: pass, error: insertErr } = await sb
      ...
      .select('id, pass_token, otp_code, visitor_name, expected_time_from, expected_time_to, max_uses')
      .single();

-- 088_consolidate_assets.sql
-- Consolidate System B (assets table, migrations 083-087) into System A
-- (infrastructure_assets table, migration 010).
--
-- Steps:
--   1. Extend infrastructure_assets with all System B columns.
--   2. Expand category CHECK to unified lowercase set; normalize existing 8 rows.
--   3. Add status column; populate from is_active.
--   4. Delete the 8 placeholder records (no make/model — will be replaced by real data).
--   5. Re-seed 70 real assets into infrastructure_assets.
--   6. Retarget staff_activity_templates.asset_id FK → infrastructure_assets.
--   7. Remove the 'assets' feature-flag module row (from migration 087).
--   8. Drop the assets table (System B) and related objects.

BEGIN;

-- ── 1. Add System B columns to infrastructure_assets ─────────────────────────
...
ALTER TABLE infrastructure_assets
  ...
  ADD COLUMN IF NOT EXISTS amc_vendor      varchar(200),   -- free-text; amc_vendor_id FK remains for formal vendor links
  ADD COLUMN IF NOT EXISTS location_id     uuid REFERENCES locations(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS location_notes  varchar(200),
...
CREATE INDEX IF NOT EXISTS idx_infra_assets_amc_expiry
  ON infrastructure_assets(amc_end) WHERE amc_end IS NOT NULL;

-- ── 4. Delete the 8 placeholder records (no make / serial — replaced by real data) ───

DELETE FROM infrastructure_assets
WHERE name IN (
  'Block A Lift','Block B Lift','DG Set 1',
  'WTP System','STP System','CCTV Network',
  'Main Gate','Fire Suppression'
) AND make IS NULL AND serial_number IS NULL;

-- ── 5. Seed 70 real assets ────────────────────────────────────────────────────
...
INSERT INTO infrastructure_assets
  (society_id, name, asset_code, category, make, model, serial_number,
   ...
   amc_vendor, amc_start, amc_end, is_active, status)
SELECT
  soc.id,
...
CROSS JOIN (VALUES
  ...
  ('Raising Main Tap-Up Box — A & B',    'EL-032','electrical', NULL,             NULL,             NULL,               NULL,       'A-Block',             'Ground floor, A and B Block',     2, NULL,        NULL,         NULL,         NULL),
  ...
  ('Floor Tap-Up Box — A & B Block',     'EL-034','electrical', NULL,             NULL,             NULL,               NULL,       'A-Block',             'Ground to 7th floor, A and B',    42,NULL,        NULL,         NULL,         NULL),
...
-- ── 6. Retarget staff_activity_templates.asset_id → infrastructure_assets ────
-- Nullify stale UUIDs (they pointed at the now-dropped assets table).
UPDATE staff_activity_templates SET asset_id = NULL WHERE asset_id IS NOT NULL;

ALTER TABLE staff_activity_templates
  DROP CONSTRAINT IF EXISTS staff_activity_templates_asset_id_fkey;

ALTER TABLE staff_activity_templates
  ADD CONSTRAINT staff_activity_templates_asset_id_fkey
  FOREIGN KEY (asset_id) REFERENCES infrastructure_assets(id) ON DELETE SET NULL;

-- ── 7. Remove the 'assets' feature-flag module row ────────────────────────────
DELETE FROM feature_flags WHERE module_key = 'assets';

-- ── 8. Drop System B (assets table) ─────────────────────────────────────────

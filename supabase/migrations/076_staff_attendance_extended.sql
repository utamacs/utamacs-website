...
SELECT
  s.id, r.rule_code, r.description, r.value_type, r.current_value, r.is_locked
FROM societies s
CROSS JOIN (VALUES
  ('STAFF_CHECKIN_QR_EXPIRY_SECS', 'Seconds a self-generated QR token is valid for check-in', 'integer', '86400', false),
  ('STAFF_OVERTIME_THRESHOLD_MINS', 'Minutes beyond shift end before overtime is counted', 'integer', '30', false)
) AS r(rule_code, description, value_type, current_value, is_locked)

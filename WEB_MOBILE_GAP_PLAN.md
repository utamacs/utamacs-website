...
- [ ] 🔴 Sub-category dropdown — dynamic, loaded per category from `GET /api/v1/complaints/sub-categories?category=X`
...
- [ ] 🟡 10-category filter tabs (Bylaws, Minutes, Financial, Legal, Circulars, Governance, HOTO, Maintenance, Forms, Other)
...
- [ ] 🟠 Cancel booking — for pending / confirmed / waitlisted bookings, with optional reason input
...
- [ ] 🔴 HOTO item detail screen — full status workflow: NOT_STARTED → IN_PROGRESS → UNDER_REVIEW → PENDING_SECRETARY / PENDING_PRESIDENT → APPROVED / REJECTED → CLOSED
- [ ] 🔴 Snag ↔ HOTO bidirectional linking (attach snag to HOTO item; block HOTO review if snag open)
- [ ] 🟠 Status transition buttons (advance / submit for review / approve / reject)
...
- [ ] 🔴 Overview KPI cards — total members, units, dues collection %, complaints, visitors
...
- [ ] 🔴 Staff directory screen — department-grouped list with role, status, contact, designation
- [ ] 🔴 KYC onboarding — photo upload, ID type/number, annual renewal tracking
...
- [ ] 🟠 Policy detail / full-content view — render HTML body, embed PDF iframe, YouTube/video player
...
- [ ] 🔴 Public self-registration form — Step 1 (name / email / phone / occupancy_type), Step 2 (flat / move-in date / ID type+number / vehicle), Step 3 (DPDPA consent review + submit)
...
- [ ] 🔴 AGM detail screen — resolutions list with votes_for / votes_against / votes_abstain / passed_at
...
- [ ] 🔴 Add tenant KYC form (exec/owner) — unit, full_name, tenancy_start/end, aadhaar_last4, monthly_rent, PAN, notes
